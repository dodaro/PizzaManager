	...
	$(document).ready(function() {
		...
		$('#resumeTablePizza tbody').on('click', 'tr', function() {
			...
			if ($(this).hasClass('selected')&& tablePizza.row(this).data()!=undefined &&($(this).hasClass("odd")||$(this).hasClass("odd")|| $(this).hasClass("even"))) {
				...
				console.log(tablePizza.row(this).data());
				tablePizza.$('tr.selected').removeClass('selected');
				$(this).addClass('selected');
				$("#editButtonPizza").prop('disabled', false);
				$("#removeButtonPizza").prop('disabled', false);
				$("#confermeButtonPizza").prop('disabled', true);
		...
		$('#resumeTableBeverage tbody').on('click', 'tr', function() {
			...
			if ($(this).hasClass('selected') && tableBeverage.row(this).data()!=undefined && ($(this).hasClass("odd")||$(this).hasClass("odd")|| $(this).hasClass("even")) ) {
				...
				tablePizza.$('tr.selected').removeClass('selected');
				$(this).addClass('selected');
				$("#editButtonBeverage").prop('disabled', false);
				$("#removeButtonBeverage").prop('disabled', false);
				$("#confermeButtonBeverage").prop('disabled', true);
				//load value relative to the selected row
				var number=tableBeverage.row($(this)).data()[4];
...
/*****************************************************   FUNZIONI COMUNI ********************************************************************************/
	var sendOrder = function(){
		...
		console.log(beverageList);console.log(orderBeverages);
		console.log(pizzaList);console.log(orderPizzas);
		var stringP=JSON.stringify(orderPizzas);
		var stringB=JSON.stringify(orderBeverages);
		$.ajax({
			url : "/pizzerialiveorderConferme",
			type : 'POST',
			data : {pizzas: stringP, beverages: stringB},
		...
		for (var int = 0; int < tablePizza.rows().count(); int++) {
			...
			for (var int2 = 0; int2 < pizzaList.length; int2++) {
				if (dataRow[5] == pizzaList[int2].getCode()) {
					orderPizzas.push({number:dataRow[4], pizza:pizzaList[int2]})
				}				
			}
		}
		return orderPizzas;
	...
	var loadIngredientsForPizza = function(pizzaName){
		...
		for (var int = 0; int < pizzeFromServer.length; int++) {
			if(pizzeFromServer[int].name==pizzaName){
				...
				var difference=_.difference(pizzeriaIngredients, pizzeFromServer[int].ingredients);
				console.log(difference);
				for (var int2 = 0; int2 < difference.length; int2++) {
					$("#addIngredients.js-example-basic-multiple").append("<option value="+pizzaName+"_"+int2+">"+difference[int2]+"</option>")
				}
				...
				for (var int2 = 0; int2 < pizzeFromServer[int].ingredients.length; int2++) {
					$("#removeIngredients.js-example-basic-multiple").append("<option value="+pizzaName+"_"+int2+">"+pizzeFromServer[int].ingredients[int2]+"</option>")
				}
			}	
		}
	}
...
/*************** FUNZIONI UTILITA SOLO PER BEVANDA *****************************************************************************************************************/	
		...
		for (var int = 0; int < tableBeverage.rows().count(); int++) {
			...
			for (var int2 = 0; int2 < beverageFromServer.length; int2++) {
				if(beverageFromServer[int2].id==id){
					...
					orderBeverages.push({number:dataRow[4], beverage:beverageFromServer[int2]});
				}
			}
		}
	...
	var createBeverage = function(beverages){
		...
		$.each( beverages, function(type) {
			beverages[type]=_.groupBy(beverages[type], 'brand');
			var int=0;
			$.each( beverages[type], function(brand) {
				...
				$.each( beverages[type][brand], function(name) {
					...
					$.each( beverages[type][brand][name], function(container) {
						...
						$("#beverageList").append("<option class='"+type+"' value='idBev_"+currentBeverage[0].id+"'>"+brand+" "+name+" - "+container+" - size:"+currentBeverage[0].size+"</option>");
						int++;
					});
				});
			});
	...
	var resolveBeverage = function(editing){
		...
		if(editing===true){
			...
			var indexToRemove=tableBeverage.row($('#resumeTableBeverage tbody > tr.odd.selected,tr.even.selected')).data()[5];
	...
	var editResumeBeverageItem = function(idBeverage, number,loaded){
		...
		for (var int = 0; int < tableBeverage.rows().count(); int++) {
			...
			if (dataRow[5] == idBeverage) {
				found = true;
				console.log("trovato il tr con idBeverage:"+dataRow[5]+", e numero di bevande: "+dataRow[4]);
				...
				for (var int2 = 0; int2 < tableBeverage.rows().count(); int2++) {
					console.log(tableBeverage.row(int2).data()[5] +"=="+rows[int][5])
...
<link href="resources/css/pageCSS/pizzerialiveorder.css" rel="stylesheet" />
			...
			<div class="row">
				...
				<div id="content" class="col-xs-10">
							...
							<div id="pizzas">
								...
								<h4>Pizza:</h4>
									...
									<div>
										...
										<button id="confermeButtonPizza" type="submit" class="btn btn-default" onclick="resolvePizza(false)">Conferme</button>
										<button id="editButtonPizza" type="submit" class="btn btn-default" onclick="resolvePizza(true)">Update</button>
										<button id="removeButtonPizza" type="submit" class="btn btn-default" onclick="removeItem('pizza')">Remove</button>
										<hr>
									</div>	
								</div>
							</div>
						...
						<div class="col-xs-4">
							...
							<h4>Beverage:</h4>
								...
								<div id="beverageControls">
									...
									<button id="confermeButtonBeverage" type="submit" class="btn btn-default" onclick="resolveBeverage(false)">Conferme</button>
									<button id="editButtonBeverage" type="submit" class="btn btn-default" onclick="resolveBeverage(true)">Update</button>
									<button id="removeButtonBeverage" type="submit" class="btn btn-default" onclick="removeItem('beverage')">Remove</button>
								</div>
							</div>
						</div>
					</div>
					...
					<hr>
						...
						<div class="col-xs-4">
							<button id="confermeOrder" type="submit" class="btn btn-default" onclick="sendOrder()">Create Order</button>
						</div>
					</div>
				</div>
			</div>
